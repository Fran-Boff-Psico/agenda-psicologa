...
function mostrarTela(nomeTela, opcoes = {}) {
    ...
    const btnFecharPerfil = document.getElementById('btnFecharPerfilPaciente');
    if (btnFecharPerfil) {
        btnFecharPerfil.style.display = (nomeTela === 'novoPaciente' && idPacienteEditando) ? 'inline-flex' : 'none';
    }
...
window.fecharPerfilPaciente = fecharPerfilPaciente;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('pesquisaPacientes')?.addEventListener('input', () => {
...
async function abrirProntuarioSessao(pacienteId, dataISO, hora = '') {
    ...
    identificacao.innerText = `Sessão de ${formatarDataBR(criarDataLocal(dataISO))}`;
    if (bancoDados) {
        ...
        if (nome) {
            ...
            identificacao.innerText = `${nome} - Sessão de ${formatarDataBR(criarDataLocal(dataISO))}`;
        }
    }
...
async function renderizarSidebarCalendarioPaciente(pacienteId, manterPeriodoAtual = false) {
    ...
    listaScroll.innerHTML = '<div style="font-size:12px; padding:10px;">Processando período...</div>';
    ...
    try {
            ...
            if (atendeRecorrencia || excecao) {
                ...
                const textoContasPagar = contasDaData.map(conta => {
                    const categoria = conta.categoria === 'Outro' || conta.categoria === 'Manual'
                        ? 'Reg. Manual'
                        : (conta.categoria === 'Sublocacao' ? 'Sublocação' : (conta.categoria || 'Reg. Manual'));
        ...
        listaScroll.innerHTML = htmlProxe || '<div style="font-size:12px; color:#718096; padding:10px;">Nenhuma sessão encontrada neste período.</div>';
    } catch (err) {
        console.error(err);
        listaScroll.innerHTML = '<div style="font-size:12px; color:#e53e3e; padding:10px;">Erro ao processar consulta do calendário.</div>';
    }
}
...
async function gerarRelatorioFinanceiro() {
    ...
    if (!dataInicio || !dataFim) {
        resultado.innerHTML = 'Informe um período válido para gerar o relatório.';
        return;
    }
                ...
                <tbody>
                    ${linhas.map(item => `
                        <tr>
                            ...
                            <td data-label="Pagamento"><span class="${item.pago ? 'badge-pago' : 'badge-aberto'}">${item.pago ? 'Pago' : 'Em aberto'}</span></td>
...
async function salvarContaManual(tipo) {
    ...
    const descricao = tipo === 'pagar'
        ? (categoriaSelecionada === 'Outro'
            ? document.getElementById('contaPagarDescricao')?.value
            : (categoriaSelecionada === 'Sublocacao' ? 'Sublocação' : categoriaSelecionada))
...
async function carregarTelaContas(tipo) {
    ...
    lista.innerHTML = `<table class="tabela-relatorio"><thead><tr><th>Data</th><th>Paciente / Descrição</th><th>Categoria</th><th>Status do atendimento</th><th>Pagamento</th><th>Valor</th><th></th></tr></thead><tbody>${linhas.map(linha => {
...
function alternarDescricaoContaPagarOcorrencia() {
    ...
    const grupoDescricao = document.getElementById('grupoDescricaoContaPagarOcorrencia');
    const grupoValor = document.getElementById('grupoValorContaPagarOcorrencia');
    const pago = document.getElementById('contaPagarOcorrenciaPagoBox');
    const escopo = document.getElementById('boxEscopoContaPagarOcorrencia');
    if (grupoDescricao) grupoDescricao.style.display = categoria === 'Outro' ? 'block' : 'none';
    if (grupoValor) grupoValor.style.display = categoria ? 'block' : 'none';
    if (pago) pago.style.display = categoria ? 'inline-flex' : 'none';
    if (escopo) escopo.style.display = categoria ? 'block' : 'none';
}
...
function carregarContaPagarOcorrencia(pacienteId, dataISO) {
    ...
    if (escopo) escopo.value = conta?.recorrente ? 'demais' : 'somente';
...
function validarContaPagarOcorrencia() {
    ...
    if (!valor || valor <= 0 || (categoria === 'Outro' && !descricao.trim())) { alert('Informe o valor e, para a categoria Outro, a descrição da conta a pagar.'); return false; }
    return true;
}

async function salvarContaPagarOcorrencia(pacienteId, dataOriginal, novaData, escopoContaPagar, frequencia) {
    const categoria = document.getElementById('contaPagarOcorrenciaCategoria')?.value || '';
    const valor = Number(document.getElementById('contaPagarOcorrenciaValor')?.value || 0);
...
function criarPdfCompartilhavelRelatorio(textoRelatorio) {
    ...
    paginas.forEach((pagina, indice) => {
        ...
        const titulo = indice === 0 ? 'Demonstrativo Financeiro de Atendimentos' : 'Demonstrativo Financeiro de Atendimentos - Continuação';
...
async function exportarRelatorioFinanceiro() {
    ...
    if (!resultado?.querySelector('table')) {
        alert('Aguarde a atualização do relatório antes de exportar.');
        return;
    }
...
function gerarPdfRelatorio() {
    ...
    if (!resultado || !resultado.querySelector('table')) {
        alert('Gere o relatório antes de criar o PDF.');
        return;
    }
    ...
    </style></head><body><div class="cabecalho"><img class="logo" src="${logoRelatorio}" alt="Logo"><h1>Demonstrativo Financeiro de Atendimentos</h1></div><div class="filtros"><b>Paciente:</b> ${escaparHTML(paciente)}<br><b>Período:</b> ${escaparHTML(inicioBR)} a ${escaparHTML(fimBR)}</div>${resultado.innerHTML}<div class="total-final">Total: ${formatarMoeda(total)}</div><script>window.onload = () => window.print();<\/script></body></html>`);
...
function atualizarBotaoLimparPesquisaPacientes() {
    const campo = document.getElementById('pesquisaPacientes');
    const botao = document.getElementById('btnLimparPesquisaPacientes');
    if (botao) botao.hidden = !campo?.value.trim();
}
...
async function carregarPacientes() {
    ...
    try {
        ...
        console.error(err);
        lista.innerHTML = '<div>Não foi possível carregar os pacientes.</div>';
    }
}
...
window.limparPesquisaPacientes = limparPesquisaPacientes;
